...
<%
    String selQry = "select * from tbl_chat where peer_id='" + request.getParameter("pid") + "' order by chat_date_time";
...
} else if (rs.getString("student_id").equals(rs.getString("student_id"))) {
    String sel11 = "select * from tbl_student where student_id='" + rs.getString("student_id") + "'";
    ResultSet x1 = con.selectCommand(sel11);
    if (x1.next()) {
%>
<div class="chat-message is-sent">
    <img src="../Assets/Files/StudentPhoto/<%=x1.getString("student_photo")%>" alt="">
    <div class="message-block">
        <span><%=x1.getString("student_name")%></span>
        <div class="message-text"><%=rs.getString("chat_content")%></div>
    </div>
</div>
<div class="chat-message" style="margin: 0px; padding: 0px" >
